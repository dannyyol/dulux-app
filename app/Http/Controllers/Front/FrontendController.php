...
class FrontendController extends Controller
{
    ...
    public function home(){
        ...
        // CartItem::where('created_at', '<=', $formatted)->delete();
        return view("frontend.home",  compact('data'));        
    }

    public function index(){
        $cart_key = [
            'cart_key' => \Str::random(12)
        ];
        $date = new \DateTime();
        $date->modify('-60 minutes');
        $formatted = $date->format('Y-m-d H:i:s');
    ...
    public function qrMenu() {
        ...
        $data['categories'] = Pcategory::where('status', 1)->where('language_id', $defaultLang->id)->get();
        $data['defaultLang'] = $defaultLang;

        return view('front.qr-menu', $data);
    }
